...
static const size_t nvertices = 4;
...
static GLuint compile_program(void) {
	static GLchar fragment_shader_src[4096];
	FILE *f = fopen("example/fractal-frag.glsl", "r");
	fread(fragment_shader_src, 1, sizeof(fragment_shader_src), f);
	fclose(f);
...
static void render_scene(uint32_t time, mat4 mvp_matrix) {
	...
	GLint res_loc = glGetUniformLocation(gl_prog, "iResolution");
	GLint time_loc = glGetUniformLocation(gl_prog, "iTime");

	glUseProgram(gl_prog);

	glUniformMatrix4fv(mvp_loc, 1, GL_FALSE, (GLfloat *)mvp_matrix);

	glUniform2f(res_loc, display_width, display_height);
	glUniform1f(time_loc, (float)time / 1000);
...
static void render_view(struct cube_xr_view *view, uint32_t time) {
	...
	glm_rotate_y(model_matrix, 3.14, model_matrix);
